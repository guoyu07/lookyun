        ...
        <div class="col-sm-12">
            <div class="ibox float-e-margins">
                ...
                <div class="ibox-content">
                    <form id="add_article" method="post" class="form-horizontal" action="<?=\yii\helpers\Url::to(['back/update'])?>">
                        <div class="form-group">
                            <label class="col-sm-2 control-label">标题</label>
                            <div class="col-sm-10">
                                <input type="text" name="title" required="" class="form-control" value="<?=$model['title']?>">
                            </div>
                        </div>
                        <input type="hidden" name="id" required="" class="form-control" value="<?=$model['id']?>">
                        <div class="form-group">
                            <label class="col-sm-2 control-label">别名</label>
                            <div class="col-sm-10">
                                <div class="row">
                                    <div class="col-md-3">
                                        <input type="text" name="alias" required="" value="<?=$model['alias']?>" placeholder="" class="form-control">
                                    </div>
                            ...
                            <div class="col-sm-10">
                                <select class="form-control m-b" name="category_id">
                                    <?php  foreach($category_id as $k=>$val){
                                        $selt = $k==$model['category_id']?'selected':'';
                                        printf('<option value="%s" %s>%s</option>',$k,$selt,$val);
                                    } ?>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-10">
                                <textarea name="content" required="" data-provide="markdown" rows="30"><?=$model['content']?></textarea>
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="col-sm-2 control-label">文章摘要</label>
                            <div class="col-sm-10">
                                <textarea name="summary" class="form-control" rows="6" aria-required="true"><?=$model['summary']?></textarea>
                            </div>
                        </div>

                        <div class="form-group">
                            <div class="col-sm-4 col-sm-offset-2">
                                <button class="btn btn-primary"  type="submit">确定更新</button>
                            </div>
                        </div>
                    </form>
                </div>
...
<script>
    ...
    $("#sub_btn").click(function(){
        $.ajax({
            cache: false,
            type: "POST",
            url: "<?=\yii\helpers\Url::to(['back/add-article'])?>",
            data:$('#add_article').serialize(),// 你的formid
    ...
    $("input[name='alias']").blur(function(dt){
        ...
        if($.trim(alia) != '')
        {
            $.getJSON("<?=\yii\helpers\Url::to(['back/uin-alias','id'=>$model['id']])?>"+"&alias="+alia,function(rtn){
                if(rtn.code != 0)
                {
